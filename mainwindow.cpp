...
QVector<double> findMode(const QVector<double> &numbers) {
    ...
    // Count the occurrences of each number in the input QVector
    ...
    // Collect all numbers with the maximum count (the modes)
    for (const auto &key : countMap.keys()) {
...
double findMedian(const QVector<double>& values) {
    if (values.isEmpty()) {
        // Handle the case when the QVector is empty.
        return 0.0; // You can choose a different default value if needed.
    }
    ...
    if (size % 2 == 0) {
        // If the size of the QVector is even, take the average of the two middle values.
        int mid = size / 2;
        return (sortedValues[mid - 1] + sortedValues[mid]) / 2.0;
    } else {
        // If the size of the QVector is odd, simply return the middle value.
        int mid = size / 2;
